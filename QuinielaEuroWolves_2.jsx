  ...
  return (
    <div style={S.root}>
      ...
      <main style={S.main}>
        ...
        {screen==="picks"       && <AllPicksScreen players={players} results={results} officialBracket={officialBracket}/>}
  ...
  return (
    <header style={S.header}>
      ...
      <nav style={S.nav}>
        ...
        <NB active={screen==="picks"} onClick={()=>setScreen("picks")}>PICKS</NB>
        {user
...
// ─── LEADERBOARD ──────────────────────────────────────────────────────────────
// ─── ALL PICKS SCREEN ─────────────────────────────────────────────────────────
function AllPicksScreen({players,results,officialBracket}) {
  const [filterRound,setFilterRound] = useState("r16");

  function handlePrint() {
    window.print();
  }

  const matches = officialBracket[filterRound]||[];
  const playerList = Object.entries(players).sort((a,b)=>a[1].name.localeCompare(b[1].name));
    ...
    <div style={S.playWrap}>
      <style>{`
        @media print {
          ...
          #picks-print, #picks-print * { visibility: visible; }
          #picks-print { position: absolute; left: 0; top: 0; width: 100%; }
          .no-print { display: none !important; }
        }
      `}</style>

      <div style={S.lbHeader}>
        <div style={S.lbTitle}>📋 PICKS DE TODOS</div>
        <div style={S.lbSub}>Verificación pública — lo que cada jugador seleccionó</div>
      </div>

      <div style={{...S.tabs,...{flexWrap:"wrap"}}} className="no-print">
        {ROUNDS.map(r=>(
          <button key={r.key} onClick={()=>setFilterRound(r.key)}
            style={{...S.tab,...(filterRound===r.key?S.tabOn:{})}}>
            <span style={S.tabLabel}>{r.label}</span>
          </button>
        ))}
      </div>

      <button className="no-print" style={{...S.btnGhost,marginBottom:4}} onClick={handlePrint}>
        📄 GUARDAR COMO PDF / IMPRIMIR
      </button>

      <div id="picks-print">
        <div style={{fontFamily:FONT_COND,fontWeight:900,fontSize:16,color:"#E8C840",letterSpacing:3,marginBottom:12,textAlign:"center"}}>
          🐺 QUINIELA EUROWOLVES — {ROUNDS.find(r=>r.key===filterRound)?.label}
        </div>

        {matches.map((m,idx)=>{
          const t1=m.team1||"TBD", t2=m.team2||"TBD";
          const rData=(results[filterRound]||{})[m.id]||{};
          return (
            <div key={m.id} style={{background:"#0A1628",border:"1px solid #1E3A5F",borderRadius:10,padding:12,marginBottom:10}}>
              <div style={{fontFamily:FONT_COND,fontWeight:800,fontSize:13,color:"#E8C840",marginBottom:8,letterSpacing:1}}>
                #{idx+1} · {t1} vs {t2}
                {m.date && <span style={{color:"#3B82F6",fontWeight:600,marginLeft:8}}>{m.date} {m.time && `· ${m.time}`}</span>}
                {rData.winner && <span style={{color:"#22C55E",marginLeft:8}}>✅ Ganó: {rData.winner} {rData.time==="90"?"(90min)":rData.time==="et"?"(Prórroga)":rData.time==="pen"?"(Penales)":""}</span>}
              </div>
              <div style={{display:"flex",flexDirection:"column",gap:4}}>
                {playerList.length===0 && <div style={{color:"#475569",fontSize:12}}>Sin jugadores registrados</div>}
                {playerList.map(([code,p])=>{
                  const pData=(p.predictions?.[filterRound]||{})[m.id]||{};
                  const hasResult=!!rData.winner;
                  const correct=hasResult&&pData.winner===rData.winner;
                  const wrong=hasResult&&pData.winner&&pData.winner!==rData.winner;
                  return (
                    <div key={code} style={{display:"flex",alignItems:"center",gap:8,padding:"4px 8px",background:"#040D18",borderRadius:6,border:`1px solid ${correct?"#22C55E":wrong?"#EF4444":"#1E3A5F"}`}}>
                      <span style={{fontFamily:FONT_COND,fontWeight:800,fontSize:12,color:"#60A5FA",width:36,flexShrink:0}}>{code}</span>
                      <span style={{fontSize:11,color:"#94A3B8",flex:1}}>{p.name}</span>
                      {pData.winner ? (
                        <>
                          <div style={{display:"flex",alignItems:"center",gap:4}}>
                            <Flag team={pData.winner} size={16}/>
                            <span style={{fontFamily:FONT_COND,fontWeight:700,fontSize:12,color:correct?"#22C55E":wrong?"#EF4444":"#E2E8F0"}}>{pData.winner}</span>
                          </div>
                          <span style={{fontFamily:FONT_COND,fontSize:10,color:"#64748B",flexShrink:0}}>
                            {pData.time==="90"?"⚽ 90min":pData.time==="et"?"⏱ Prórroga":pData.time==="pen"?"🥅 Penales":""}
                          </span>
                          {correct && <span style={{fontSize:11}}>✅</span>}
                          {wrong   && <span style={{fontSize:11}}>❌</span>}
                        </>
                      ) : (
                        <span style={{fontFamily:FONT_COND,fontSize:11,color:"#475569"}}>— sin pick</span>
                      )}
                    </div>
                  );
                })}
        ...
        {/* Campeón */}
        {filterRound==="f" && (
          <div style={{background:"#0A1628",border:"1px solid rgba(232,200,64,0.4)",borderRadius:10,padding:12,marginBottom:10}}>
            <div style={{fontFamily:FONT_COND,fontWeight:800,fontSize:13,color:"#E8C840",marginBottom:8,letterSpacing:1}}>
              🏆 CAMPEÓN DEL MUNDO
              {results.champion && <span style={{color:"#22C55E",marginLeft:8}}>✅ {results.champion}</span>}
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:4}}>
              {playerList.map(([code,p])=>{
                const correct=results.champion&&p.predictions?.champion===results.champion;
                const wrong=results.champion&&p.predictions?.champion&&p.predictions.champion!==results.champion;
                return (
                  <div key={code} style={{display:"flex",alignItems:"center",gap:8,padding:"4px 8px",background:"#040D18",borderRadius:6,border:`1px solid ${correct?"#22C55E":wrong?"#EF4444":"#1E3A5F"}`}}>
                    <span style={{fontFamily:FONT_COND,fontWeight:800,fontSize:12,color:"#60A5FA",width:36,flexShrink:0}}>{code}</span>
                    <span style={{fontSize:11,color:"#94A3B8",flex:1}}>{p.name}</span>
                    {p.predictions?.champion ? (
                      <div style={{display:"flex",alignItems:"center",gap:4}}>
                        <Flag team={p.predictions.champion} size={16}/>
                        <span style={{fontFamily:FONT_COND,fontWeight:700,fontSize:12,color:correct?"#22C55E":wrong?"#EF4444":"#E2E8F0"}}>{p.predictions.champion}</span>
                        {correct && <span style={{fontSize:11}}>✅</span>}
                        {wrong   && <span style={{fontSize:11}}>❌</span>}
                      </div>
                    ) : (
        ...
        <div style={{fontFamily:FONT_COND,fontSize:10,color:"#475569",textAlign:"center",marginTop:8,letterSpacing:1}}>
          Generado desde quiniela-euro-wolves.vercel.app · {new Date().toLocaleDateString("es-MX",{day:"2-digit",month:"long",year:"numeric",hour:"2-digit",minute:"2-digit"})}
        </div>
      </div>
    </div>
  );
